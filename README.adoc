...
topic::
  A _topic_ is considered to be an atomic chunk of content within a file. It may or may not be self-contained. In the future, it is likely to adhere to a content model such as concept, task or reference, and may fit into larger structures such as modules/assemblies to represent use cases.
